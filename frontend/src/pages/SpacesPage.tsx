...
export default function SpacesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSpaceId) {
        // MODO EDICIÓN: Petición PATCH acoplada perfectamente a tu controlador de NestJS
        await axios.patch(`${import.meta.env.VITE_API_URL}/spaces/${editingSpaceId}`, payload);
        alert("¡Espacio actualizado con éxito!");
      } else {
        // MODO CREACIÓN: Petición POST
        await axios.post(`${import.meta.env.VITE_API_URL}/spaces`, payload);
  ...
  const handleDeleteClick = async (id: number) => {
    if (window.confirm("¿Estás seguro de que deseas eliminar este espacio por completo?")) {
      try {
        ...
        alert("No se pudo eliminar el espacio (Verifica si tiene reservas asociadas)");
      }
    }
  };

  if (loading) return <div className="p-6 text-slate-600 font-medium">Cargando espacios...</div>;

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h1 className="text-2xl font-bold text-slate-800 mb-6">Gestión de Espacios</h1>

      {/* FORMULARIO DINÁMICO */}
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow mb-8 flex flex-wrap gap-4 items-end">
        <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
          <label className="text-xs font-semibold text-slate-600">Nombre del Espacio</label>
          <input
            type="text"
            value={name}
        ...
        <div className="flex flex-col gap-1 w-32">
          <label className="text-xs font-semibold text-slate-600">Capacidad Máxima</label>
          <input
            type="number"
            value={capacity}
        ...
        <div className="flex flex-col gap-1 w-44">
          <label className="text-xs font-semibold text-slate-600">Estado Actual</label>
          <select
            value={isAvailable}
        ...
        <div className="flex gap-2">
          ...
          >
            {editingSpaceId ? "Actualizar Espacio" : "Agregar Espacio"}
          </button>
          ...
          <tbody className="text-slate-700 text-sm divide-y divide-slate-100">
            {spaces.map((space) => (
              <tr key={space.id} className="hover:bg-slate-50 transition-colors">
                <td className="p-4 font-semibold text-slate-900">{space.name}</td>
                <td className="p-4 text-slate-600">{space.capacity} personas</td>
                <td className="p-4 text-slate-600">{space.location}</td>
                <td className="p-4">
                  {/* BADGES RE-ESTILIZADOS: Únicamente Azul y Gris */}
                  <span
