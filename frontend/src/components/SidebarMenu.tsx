...
export default function SidebarMenu({
  ...
  const activeUser =
    user && Object.keys(user).length > 0 ? user : parsedLocalUser;

  const isAdmin = activeUser?.roleId === 1;
  const isStudent = activeUser?.roleId === 2;

  const userRoleText = isAdmin ? "Administrador" : "Estudiante";
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 text-white w-64 p-4 justify-between min-h-screen font-sans">
          ...
          {isAdmin && (
            <>
              <button
                ...
                className={`w-full text-left p-3 rounded-xl font-medium text-sm transition ${
                  current === "dashboard"
      ...
      <div className="pt-4 border-t border-slate-800/60">
        <button
          onClick={onLogout}
          className="w-full text-left p-3 rounded-xl font-bold text-sm bg-blue-600 text-white hover:bg-blue-700 transition flex items-center gap-2.5"
        >
